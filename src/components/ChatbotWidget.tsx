...
const ChatbotWidget = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      // 1. Enviar al webhook de entrada
      ...
      // 2. Esperar que Make termine de procesar
      await new Promise((res) => setTimeout(res, 2500));

      // 3. Obtener respuesta desde el mismo webhook con el texto
      const response = await fetch(`https://hook.us2.make.com/0ypbzvfjgcovtg72dpmx4re679fgeov2?text=${encodeURIComponent(input)}`);
